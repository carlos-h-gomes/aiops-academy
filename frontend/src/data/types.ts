...
export interface UnitProgress{lab_passed:boolean;completed:boolean;note:string;review:{due:string;interval:number;last_score:number}|null}
export interface Progress{settings:{start_date:string;daily_hours:0.5|0.75|1|1.5|2|3|5};notes:Record<string,string>;completed:number[];quizzes:Record<string,number>;labs:Record<string,boolean>;reviews:Record<string,{due:string;interval:number;last_score:number}>;unit_progress:Record<string,UnitProgress>}
