...
export const ESSENTIAL_STUDY_MINUTES=7050
export function studyDays(hours=3){return Math.ceil(ESSENTIAL_STUDY_MINUTES/(hours*60))}
...
export function minutesDate(start:string,minutes:number,hours=3){const d=new Date(start+'T12:00:00');d.setDate(d.getDate()+Math.floor(minutes/(hours*60)));return shortDate(localDate(d))}
