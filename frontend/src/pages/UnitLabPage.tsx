...
export function UnitLabPage({unitId}:{unitId:string}) {
  ...
  return <><a className="back-link" href={`#/unit/${unitId}`}><ArrowLeft size={16}/>Voltar à aula</a><PageTitle eyebrow={`LAB GUIADO · ${unitId.toUpperCase()}`} title={lab.title} description={lab.intro}/><div className="notice"><strong>Prática local sem registro</strong><p>{lab.limitations}</p><p>Suas escolhas ficam somente nesta tela e somem ao reiniciar. Elas não alteram calendário, notas, quizzes, conclusão ou backup.</p></div><form className="unit-lab-form" onSubmit={event=>{event.preventDefault();if(!unanswered)void run(answers)}}><section className="lesson-paper"><div className="objectives"><p className="eyebrow">DECIDA COM BASE NA FIXTURE</p><p><FlaskConical size={16}/>Cenário fechado · feedback explicável</p></div>{lab.questions.map((question,index)=><fieldset key={question.id} className="unit-lab-question"><legend>{index+1}. {question.prompt}</legend>{question.options.map(option=><label key={option.value} className="answer"><input type="radio" name={question.id} value={option.value} checked={answers[question.id]===option.value} onChange={()=>{setAnswers(value=>({...value,[question.id]:option.value}));clearResult()}}/>{option.label}</label>)}</fieldset>)}<div className="action-row"><button className="primary" type="submit" disabled={running||unanswered>0}>{running?'Conferindo…':unanswered?`Responda ${unanswered} item(ns)`:'Conferir decisões'}</button><button className="secondary" type="button" onClick={reset}><RotateCcw size={16}/>Reiniciar escolhas</button></div>{error&&<Message text={error} error/>}{result&&<div className={result.correct?'notice unit-lab-result':'notice error unit-lab-result'} role="status"><strong>{result.correct?'Decisões conferidas':'Há decisões a revisar'}</strong><p>{result.feedback}</p>{!result.correct&&<p>Itens: {result.reviewed_fields.join(', ')}.</p>}</div>}</section></form></>
}
