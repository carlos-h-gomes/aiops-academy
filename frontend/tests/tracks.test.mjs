...
try {
  ...
  for(const width of [1440,390,320]){
    ...
    await page.locator('#data-01').getByRole('link',{name:/Abrir aula/}).click()
    await expect(page.getByRole('heading',{name:'Modelagem relacional'})).toBeVisible()
    await expect(page.getByText('Acesso local · sem registro de progresso')).toBeVisible()
    await expect(page.getByText('Esta aula não altera calendário, notas, quizzes, conclusão ou backup.')).toBeVisible()
    await audit('data-lesson',width)
    await page.getByRole('link',{name:'Abrir lab guiado'}).click()
