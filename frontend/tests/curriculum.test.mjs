...
test('track filtering respects accents, status, ordering and input immutability',()=>{
  ...
  assert.equal(trackUnits(catalog.units,'data','','available').length,0)
  assert.equal(trackUnits(catalog.units,'data','','planned').length,6)
...
test('legacy completed days project once without modifying study state',()=>{
  ...
  assert.deepEqual(summary,{available:30,planned:0,complete:2,essentialMinutes:5400})
...
test('planned tracks have no completed units or fictitious duration',()=>{
  const units=trackUnits(catalog.units,'agents')
  assert.deepEqual(trackSummary(units,progress),{available:0,planned:8,complete:0,essentialMinutes:0})
  assert.equal(unitComplete({...units[0],lesson_day:1},progress),false)
})
